...
## TRELLO BOARD
* [TRELLO: Lambda Notes - Jean S Ariza](https://trello.com/b/ntqeFPwr "TRELLO: Lambda Notes - Jean S Ariza")

# Dev Server:
* This server.js is aim to facilitate the Dev of the Lmabda-Notes App.
* This server is a copy refactored from previous projects.
  * **[GET]** to `/api/notes`: returns the list of notes.
  * **[GET]** to `/api/notes/:id`: returns the note with the id passed as part of the URL (123 in example).
  * **[POST]** to `/api/notes`: creates a note and return the new list of notes. Pass the note as the `body` of the request (the second argument passed to `axios.post`).
  * **[PUT]** to `/api/notes/:id`: updates the note using the `id` passed as part of the URL. Send the an object with the updated information as the `body` of the request (the second argument passed to `axios.put`).
  * **[DELETE]** to `/api/notes/:id`: removes the note using the `id` passed as part of the URL (123 in example).
