...
class NoteList extends React.Component {
  ...
  render() {
    ...
    notes.forEach(note => {
      ...
      if (
        note.content.length > 87 &&
        note.content === note.content.toUpperCase()
      ) {
        note.content = note.content.slice(0, 87) + "...";
      }
    });

    const draggableNotes = notes
      .map(note => {
        if (this.state.isSortable) {
          return (
            <div className="note-preview-container" key={Math.random()}>
              <h3>
                <Markdown escapeHtml={true} source={note.title} />
              </h3>
              <p>
                <Markdown escapeHtml={true} source={note.content} />
              </p>
            </div>
          );
        } else {
    ...
    return (
      <div className="main-container">
        ...
        <div className="note-previews-container">
          <DragSortableList items={draggableNotes} type="grid" />
        </div>
      </div>
    );
  }
