...
export default (state = initialState, action) => {
	switch (action.type) {
		...
		case GOT_NOTES:
			return {
				...state,
				status: { ...state.status, gettingNotes: false, gotNotes: true },
				notes: action.payload,
				noteViewer: noteViewerInitial
			};
		case GETTING_SINGLE_NOTE:
			return {
				...state,
				status: { ...state.status, gettingSingleNote: true, gotSingleNote: false, isEditing: false, noteMessage: '', noteError: '' },
				noteViewer: noteViewerInitial
			};
		case GOT_SINGLE_NOTE:
			return {
				...
				status: {
					...
					gettingSingleNote: false,
					gotSingleNote: true,
					noteError: action.payload.errorMessage ? action.payload.errorMessage : ''
				},
				noteViewer: { ...action.payload }
			};
		//~~~~~PUT Handling~~~~~//
		...
		case DELETED_NOTE:
			return {
				...state,
				status: { ...state.status, deletingNote: false, deletedNote: true, deletePrompt: false, noteMessage: action.payload.success },
				noteViewer: noteViewerInitial
			};
		//~~~~~default~~~~~//
		default:
