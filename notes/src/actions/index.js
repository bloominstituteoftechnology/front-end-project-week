...
export const getNotes=(options)=>{
  const request=axios.get('https://notes-lambda.herokuapp.com/api/notes',options);
...
export const getSingleNote=(id)=>{
    const request=axios.get(`https://notes-lambda.herokuapp.com/note/get/${id}`);
    return function (dispatch){
...
export const addNote=(newNote,history)=>{
  const request=axios.post('https://notes-lambda.herokuapp.com/note/create',newNote);
  return function (dispatch){
...
export const deleteNote=(id,history)=>{
  const request= axios.delete(`https://notes-lambda.herokuapp.com/note/delete/${id}`);
  return function (dispatch) {
...
export const updateNote=(id,editedNote,history)=>{
  const request=axios.put(`https://notes-lambda.herokuapp.com/note/edit/${id}`,editedNote)
  return function (dispatch) {
