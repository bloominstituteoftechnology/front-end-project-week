...
class NotesContainer extends React.Component {
  ...
  render() {
    ...
    if (search.length > 0) {
      notes = notes.filter(
        note =>
          note.title.toLowerCase().match(search) ||
    ...
    return (
      <React.Fragment>
        {this.props.fetching ? (
          <Loading message="Fetching Notes..." />
        ) : (
          <NotesWrapper>
            <MainNotesHeaderContainer>
              <ModalContainer
...
export default RequireAuth(
  connect(
    mapStateToProps,
    { getNotes, setOrder, deleteNote, setNotes }
  )(DragDropContext(HTML5Backend)(NotesContainer))
);
