  ...
  handleSearch = () => {
    ...
    searches.forEach(searchTerm => {
      results.forEach(result => {
        if (
          result.tags.includes(searchTerm) ||
          result.title.includes(searchTerm) ||
          result.textBody.includes(searchTerm)
        ) {
    ...
    return (
      <div className="main-container">
        ...
        <div className="create-note-form searching-form">
          ...
          {notes.map((note, i) => (
            <Link
              to={{
                ...
                state: this.props.notes.filter(nt => nt.title === note)[0]
              }}
              key={note}
            >
              <div className="title-search" key={i}>
                {note}
              </div>
            </Link>
          ))}
          <Route
            path={`/search/:id`}
            render={props => <SearchFullNote {...props} />}
          />
        </div>
      </div>
    );
